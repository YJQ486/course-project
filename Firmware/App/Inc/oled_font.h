/**
 * @file    oled_font.h
 * @brief   6x8 ASCII 点阵字库（可打印字符 0x20~0x7E，每字符 6 字节，列向取模）
 *
 * 说明：本表为各开源 OLED 教程中广泛使用的标准 6x8 ASCII 字库，用于点亮
 * 验证与英文/数字界面显示。若需中文或大号时间字体，请用 PCtoLCD2002 等
 * 取模软件按“列行式、逆向”重新生成后追加替换，无需改动驱动逻辑。
 *
 * 取模方式：每个字符 6 列，每列 1 字节 = 8 行（bit0 在顶部，bit7 在底部）。
 */
#ifndef OLED_FONT_H
#define OLED_FONT_H
