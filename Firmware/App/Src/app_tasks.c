 ...
 *
 * 任务划分（对齐《系统设计文档》）：
 *   Task_Power(4)  : 500ms 周期检查空闲，超时关屏（仅关 OLED，不进 STOP）
...
/* 若步数较上次落盘有变化则写入 Flash；force=1 用于熄屏前强制落盘。
...
void App_Init(void)
{
    ...
    MPU6050_Init();
    /* 抬腕唤醒：阈值 15(约60mg)/持续 3ms——对抬腕足够灵敏、又能滤掉瞬时噪声。
...
/* ====================== 熄屏管理（仅关 OLED，不进 STOP） ====================== */
/* 旧方案使用 STOP 模式 + EXTI 唤醒，但存在致命的 I2C 总线竞争：
 *   Task_Power（优先级4）抢占 Task_Sensor（优先级3）的 I2C 事务时，
 *   enter_stop_mode() 里 MPU6050_ReadIntStatus() 因 I2C BUSY 返回失败，
 *   锁存中断标志未清除，INT 线保持高电平，STOP 中无新上升沿，抬腕不唤醒。
 *
 * 新方案：熄屏只关 OLED，MCU 全速运行。EXTI 始终被 ISR 正常服务，
 * 运动中断在熄屏时调用 mark_activity() 即可亮屏，彻底消除竞争。
 * 代价是功耗升高（约30mA vs STOP 约20uA），但对课程演示无影响，
 * 且低功耗电源管理不在课程考核标准内。 */
...
static void Task_Power(void *arg)
{
    ...
    for (;;) {
        if (s_screen_off) {
            /* 熄屏中：运动中断/按键/蓝牙调用了 mark_activity 后，活动时间被刷新，
             * 此条件不再成立，执行亮屏。最多 500ms 延迟，对抬腕体验无感知。 */
...
static void Task_Sensor(void *arg)
{
    ...
    for (;;) {
        ...
        if (MPU6050_Read(&d) == 0) {
            i2c_fail = 0;
            /* 读 INT_STATUS 重新"布防"锁存的运动中断：把锁存的高电平清掉，
             * 使下一次动作能再产生上升沿（锁存模式下 INT 高电平保持到读 0x3A
             * 才清除，不清则 INT 线不回落、EXTI 无新上升沿、计数不再增长）。 */
...
static void Task_Comm(void *arg)
{
    ...
    for (;;) {
        /* RX 看门狗：HAL_UART_Transmit（阻塞式）持锁期间若收到字节，
         * RxCpltCallback 里的 HAL_UART_Receive_IT 会返回 BUSY，接收断链。
         * 缩短信号量等待到 100ms，让看门狗更频繁地重挂接收。 */
        if (huart2.RxState == HAL_UART_STATE_READY) {
            HAL_UART_Receive_IT(&huart2, &s_uart_rx, 1);
        }

        /* 等待一整行命令（最多 100ms），超时则绕回看门狗检查 + 状态推送 */
        if (xSemaphoreTake(s_sem_uart, pdMS_TO_TICKS(100)) == pdTRUE) {
            uint8_t h, m, s;
...
/* EXTI：编码器按键 -> 记录活动；MPU6050 运动中断 -> 熄屏时唤醒亮屏。
 * 亮屏时运动中断只计数（避免佩戴微动反复刷新活动时间导致永不熄屏）；
 * 熄屏时运动中断额外调用 mark_activity()，Task_Power 下个周期检测到
 * 活动时间被刷新即执行 exit_idle() 亮屏。MCU 全速运行，EXTI 始终可响应。 */
